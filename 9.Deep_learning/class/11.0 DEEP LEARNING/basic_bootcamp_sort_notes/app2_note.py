...
- 📚 Theory panels on each tab for students
"""
...
def load_mnist(subsample_train=20000, subsample_test=5000):
    """
    Load MNIST dataset and optionally subsample it for faster demos.
    """
...
def build_ann_model(
    ...
    """
    Build a configurable ANN for MNIST.
    """
    model = Sequential()
...
with tabs[0]:
    ...
    with st.expander("📚 Theory: What is MNIST & How is Data Represented?", expanded=False):
        st.markdown(
            """
            **MNIST Dataset**
            - Contains 70,000 images of handwritten digits (0–9).
            - Each image is **28 × 28 pixels**, grayscale.
            - Input to the ANN: 784 features (28×28 flattened into a 1D vector).
            - Output: one of **10 classes** (digit 0 to 9).

            **Why MNIST is used for teaching?**
            - Simple and small.
            - Easy to visualize.
            - Good starting point for understanding classification with neural networks.

            **Preprocessing Steps**
            - Flatten images: `(28, 28) → (784,)`
            - Normalize pixel values to `[0, 1]` by dividing by 255.
            - Convert labels to **one-hot vectors**, e.g.:
...
with tabs[1]:
    ...
    with st.expander("📚 Theory: ANN, Activations, Loss, Optimizer, Regularization, Callbacks", expanded=False):
        st.markdown(
            """
            ### Artificial Neural Network (ANN)
            - An ANN is a stack of layers that transforms input data into outputs.
            - Each layer has **neurons** with **weights + bias + activation function**.

            **Architecture here**
            - Input: 784 features (flattened image).
            - Hidden layers: configurable (1–3) with configurable units.
            - Output: 10 neurons with **Softmax** → probability for each digit.

            ### Activation Functions
            - **ReLU**: `max(0, x)` – default for hidden layers, fast & works well.
            - **tanh** / **sigmoid**: older activations; can cause slower training or vanishing gradients.
            - **Softmax** (output): converts logits into probabilities for multi-class classification.
            ...
            ### Regularization
            - **L2**: penalizes large weights to avoid overfitting.
            - **Dropout**: randomly drops neurons during training so network cannot “memorize”.

            ### Callbacks
            - **EarlyStopping**: stop training when validation loss stops improving.
            - **ReduceLROnPlateau**: reduce learning rate when learning slows down.
            """
        )
...
with tabs[2]:
    ...
    with st.expander("📚 Theory: What Are Hyperparameters & Grid Search?", expanded=False):
        st.markdown(
            """
            ### Hyperparameters
            - Values we choose **before** training (not learned by the model).
            - Examples:
              - Learning rate
              - Batch size
              - Number of layers
            ...
            ### Why Search Hyperparameters?
            - Different values can give very different accuracy.
            - We usually don't know the best combination in advance.

            ### Grid Search
            - Pick a **small set of values** for each hyperparameter.
            - Train a model for **each combination**.
            - Compare validation accuracy and choose the best.
            - This tab shows a **mini grid search** over:
              - Learning rate
              - Batch size
            """
        )
...
with tabs[3]:
    ...
    with st.expander("📚 Theory: Optuna & Automated Hyperparameter Optimization", expanded=False):
        st.markdown(
            """
            ### What is Automated Hyperparameter Optimization?
            - Instead of manually trying combinations, we let an **algorithm** search.
            - Goal: find a set of hyperparameters that gives the **best performance**.

            ### Optuna
            - A popular Python library for **hyperparameter optimization**.
            - Uses smart search strategies (like **Tree-structured Parzen Estimator**) instead of brute-force grid.
            - Key ideas:
              - Define an **objective function** that:
                - Builds a model using trial-suggested hyperparameters.
                - Trains it.
                - Returns a score (e.g. validation accuracy).
              - Optuna runs many **trials**, each with different hyperparameters.
              - Keeps track of the best trial.

            ### Difference vs Grid Search
            - Grid Search:
              - Tries all combinations in a fixed grid.
              - Can be slow and wasteful.
            - Optuna:
              - Learns from past trials.
              - Focuses on promising regions of hyperparameter space.
              - More efficient for **deep learning**.
            """
        )
