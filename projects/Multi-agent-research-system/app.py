...
def save_credentials(tavily_key: str, openai_key: str):
    env_path = Path(".env")
    env_lines = env_path.read_text().splitlines() if env_path.exists() else []
    new_lines = []
    keep_keys = {"TAVILY_API_KEY", "OPENAI_API_KEY"}
    ...
    new_lines.append(f'TAVILY_API_KEY="{tavily_key.strip()}"')
...
if page == "Credentials":
    ...
    with st.form("credentials_form"):
        st.text_input("TAVILY_API_KEY", value=tavily_default, placeholder="Enter your Tavily API key", key="tavily_input")
        st.text_input("OPENAI_API_KEY", value=openai_default, placeholder="Enter your OpenAI API key", key="openai_input")
        save_btn = st.form_submit_button("Save Credentials")

    if save_btn:
        if not st.session_state.tavily_input.strip() or not st.session_state.openai_input.strip():
            st.warning("Both credentials are required.")
        else:
            save_credentials(st.session_state.tavily_input, st.session_state.openai_input)
            st.success("Credentials saved to .env. Switch back to Home and rerun the pipeline.")

    st.markdown(
        "<div style='margin-top:1rem;color:#a09890;'>You can paste your keys here and they will be stored in your local <code>.env</code> file.</div>",
        unsafe_allow_html=True,
    )
    st.stop()
...
if page == "Home":
    if st.button("Add credentials", use_container_width=True):
