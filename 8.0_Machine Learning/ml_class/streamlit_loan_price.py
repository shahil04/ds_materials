...
if st.button("Predict Loan Amount"):
    ...
    input_data = pd.DataFrame(
        ...
        columns=["ApplicantIncome", "CoapplicantIncome", "Loan_Amount_Term", "Credit_History"]
    )
    
    # Predict loan amount
    predicted_loan_amount = model.predict(input_data)[0]
    
    st.success(f"Predicted Loan Amount: {predicted_loan_amount}")
...
# run
# python -m streamlit run streamlit_loan.py      